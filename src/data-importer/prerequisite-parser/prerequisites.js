...
export async function parsePrerequisite(fastify, requirement, requisiteType, parentPrerequisiteId, parentCourseId, coreqId) {
    ...
    const programAntireq = /^Not open to students enrolled in ((?:(?:\s*or\s*|\s*)a<[^>]*>,?)+)$/g.exec(requirement);
    // Couple edge cases is dealt w here:
    // Honours Life Physics and "program offered by all faculties"
    const pseudoProgramPrereq = /^Enrolled in(?! a program offered by all| a Bachelor).*(?:Honours(?! Life Physics)|Facult|co-operative).*$/g.exec(requirement);
    const pseudoProgramAntireq = /^Not open to students (?! a program offered by all).*(?:Honours(?! Life Physics)|Facult|co-operative).*$/g.exec(requirement);
    const degreePrereq = /^Enrolled in\s*(?:a|)\s*((?:\s*or\s*|)BASc|Bachelor of .*?)\s*(?:program|)\s*(?:in the (Faculty of .*)|)$/g.exec(requirement);
    const degreeAntireq = /^Not open to students enrolled in (Bachelor of .*?)\s*(?:programs|)$/g.exec(requirement);
    if (coursePrereq) {
        ...
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { amount: amount, requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId }));
        return {
            parentPrerequisiteId: parentPrerequisiteId,
            coreqId: coreqId,
            requisiteType: requisiteType,
        ...
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { amount: 0, requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId }));
        return {
            parentPrerequisiteId: parentPrerequisiteId,
            coreqId: coreqId,
        ...
        for (const program of programs) {
            if (program === "Experiential Education (EDGE) Certificate" ||
        ...
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, amount: 0 }));
        ...
        const pseudoProgramText = pseudoProgramPrereq[0]
            .replace(/Enrolled in (?:an|a program offered by\s*(?:the|)|a program in|a|)\s*/g, "")
            .replace(/(?:\s*(?:program|plan)|(?:Faculty of|Faculties of)\s*)/g, "");
        const pseudoPrograms = pseudoProgramText.split(/\s*(?:, or|or a(?:n|)|or|,)\s*/);
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, amount: 1 }));
        let majorType = null;
        let majorSystem = null;
        for (const pseudoProgram of pseudoPrograms) {
            if (pseudoProgram === "Mathematics/BASE") {
                ...
                if (pseudoProgram.includes("Honours"))
                    majorType = "H";
                ...
                db.insertPseudoProgramPrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoProgramPrerequisite), { requisiteType: "prereq", requisiteSubtype: "pseudoProgram", parentPrerequisiteId: parentPrerequisiteId, faculty: facultyCode, majorType: majorType, majorSystem: majorSystem }));
            }
        }
        return {
            parentPrerequisiteId: null,
        ...
        const pseudoProgramText = pseudoProgramAntireq[0]
            .replace(/Not open to students\s*(?:enrolled|)\s*in (?:an|a program offered by\s*(?:the|)|a program in|a|the|programs offered by\s*(?:the|)|)\s*/g, "")
            .replace(/(?:\s*(?:program(?:s|)|plan)|(?:Faculty of|Faculties of)\s*)/g, "");
        const pseudoPrograms = pseudoProgramText.split(/\s*(?:, or|or a(?:n|)|or|,)\s*/);
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, amount: 0 }));
        let majorType = null;
        let majorSystem = null;
        for (const pseudoProgram of pseudoPrograms) {
            if (pseudoProgram === "Mathematics/BASE") {
                ...
                const facultyCode = facultyName === ""
                    ? null
                    : await db.searchFaculties(fastify, facultyName);
                db.insertPseudoProgramPrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoProgramPrerequisite), { requisiteType: "antireq", requisiteSubtype: "pseudoProgram", parentPrerequisiteId: parentPrerequisiteId, faculty: facultyCode, majorType: majorType, majorSystem: majorSystem }));
            }
        }
        return {
            parentPrerequisiteId: null,
    ...
    // Edge case time!!
        ...
        db.insertPseudoProgramPrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoProgramPrerequisite), { requisiteType: "prereq", requisiteSubtype: "pseudoProgram", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, faculty: "ENG" }));
        return {
            parentPrerequisiteId: null,
            coreqId: coreqId,
            requisiteType: null,
        ...
        db.insertPseudoCoursePrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoCoursePrerequisite), { requisiteType: "antireq", requisiteSubtype: "pseudoCourse", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, subject: "CS", minCatalogNumber: 200, maxCatalogNumber: 499 }));
        return {
            parentPrerequisiteId: null,
            coreqId: coreqId,
            requisiteType: null,
        ...
        db.insertPseudoProgramPrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoProgramPrerequisite), { requisiteType: "antireq", requisiteSubtype: "pseudoProgram", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, faculty: "MAT" }));
        return {
            parentPrerequisiteId: null,
            coreqId: coreqId,
            requisiteType: null,
        ...
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, amount: 1 }));
        db.insertPseudoProgramPrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoProgramPrerequisite), { requisiteType: "antireq", requisiteSubtype: "pseudoProgram", parentCourseId: null, parentPrerequisiteId: parentPrerequisiteId, faculty: "MAT" }));
        db.insertPseudoCoursePrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoCoursePrerequisite), { requisiteType: "antireq", requisiteSubtype: "pseudoCourse", parentCourseId: null, parentPrerequisiteId: parentPrerequisiteId, subject: "BUS", catalogNumber: "231W" }));
        return {
            parentPrerequisiteId: null,
            coreqId: coreqId,
            requisiteType: null,
    ...
    else if (requirement === "Not open to Arts and Business students in level 1A or 1B") {
        const programId = (await db.searchPrograms(fastify, "Arts and Business"))[0][0];
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, amount: 1 }));
        db.insertProgramPrerequisites(fastify, {
            requisiteType: "antireq",
            requisiteSubtype: "program",
        ...
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: null, parentPrerequisiteId: parentPrerequisiteId, amount: 0 }));
        db.insertLevelPrerequisites(fastify, {
            requisiteType: "antireq",
            requisiteSubtype: "level",
            parentCourseId: null,
        ...
        const programIds = requirement.includes("Chemical Engineering")
            ...
            : requirement.includes("Biotechnology/Chartered Professional Accountancy")
                ? await db.searchPrograms(fastify, "Biotechnology/Chartered Professional Accountancy")
        ...
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, amount: 1 }));
        db.insertPseudoProgramPrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoProgramPrerequisite), { requisiteType: "antireq", requisiteSubtype: "pseudoProgram", parentCourseId: null, parentPrerequisiteId: parentPrerequisiteId, faculty: "SCI" }));
        for (const programId of programIds) {
            db.insertProgramPrerequisites(fastify, {
                requisiteType: "prereq",
                requisiteSubtype: "program",
        ...
        programIds.push(await util.convertProgramName(fastify, "H-Materials and Nanosciences"));
        parentPrerequisiteId = await db.insertParentPrerequisites(fastify, Object.assign(Object.assign({}, nullParentPrerequisite), { requisiteType: "prereq", requisiteSubtype: "parent", parentCourseId: parentCourseId, parentPrerequisiteId: parentPrerequisiteId, amount: 1 }));
        db.insertPseudoProgramPrerequisites(fastify, Object.assign(Object.assign({}, nullPseudoProgramPrerequisite), { requisiteType: "antireq", requisiteSubtype: "pseudoProgram", parentCourseId: null, parentPrerequisiteId: parentPrerequisiteId, faculty: "SCI" }));
        for (const programId of programIds) {
            db.insertProgramPrerequisites(fastify, {
