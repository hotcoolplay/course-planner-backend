...
import { prerequisiteTexts } from '../../app'
...
export async function scrapeDegrees(fastify: FastifyInstance, browser: Browser, link: string): Promise<DegreeProperties> {
    ...
    const degreeName = await degreePage.$eval(nameSelector, (el: Element) => el.textContent)
    if (degreeName) {
...
export async function scrapePrograms(fastify: FastifyInstance, browser: Browser, link: string): Promise<ProgramProperties> {
    ...
    const name = await programPage.$eval(nameSelector, (el: Element) => el.textContent)
    if (name) {
        let subtype: programSubtype
        if (name.includes('Bachelor') || name.includes('Joint Honours') 
            || name.includes('Doctor') || name === 'Arts and Business') subtype = 'Major'
        else if (name.includes('Diploma')) subtype = 'Diploma'
        else if (name.includes('Minor')) subtype = 'Minor'
        else if (name.includes('Option')) subtype = 'Option'
        else if (name.includes('Specialization')) subtype = 'Specialization'
        else {
            programPage.close()
            return { name: null, programSubtype: null, urlCode: null }
        }
        programPage.close()
        const programSubtype: programSubtype = subtype
...
export async function scrapeCourse(fastify: FastifyInstance, browser: Browser, page: Page, subject: string, catalog: string): Promise<CourseProperties> {
    ...
    if (!button) return {units: null, completions: null, simulEnroll: null}
    const expanded = await page.evaluate((el: Element) => el.getAttribute('aria-expanded'), button)
    if (expanded === 'false') {
    ...
    try {
        ...
        const link = await page.$eval(linkSelector, (el: Element) => (<HTMLAnchorElement>el).href)
...
async function pullText(page: Page, heading: string, wait: boolean): Promise<string | null> {
    const selector = `::-p-xpath(//*[preceding-sibling::h3[contains(., '${heading}')]]//text())`
    if (wait) {
        try { 
            await page.waitForSelector(selector)
            const element = await page.$(selector)
            return !element ? null : await page.evaluate((el: Element) => el.textContent, element)
        } catch (err: any) {
            return null
        }
    }
...
async function cleanText(el: ElementHandle): Promise<string | null> {
...
async function returnElement(page: Page, heading: string) {
