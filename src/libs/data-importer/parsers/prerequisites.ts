...
async function parsePrerequisite(fastify: FastifyInstance, requirement: string, 
    requisiteType: requisiteType, id: number, coreqId: number): Promise<(number | null)[]> {
    ...
    if (prereq1) {
        ...
        const newId = await db.insertPrerequisites(fastify,
            { ...nullPrerequisite, amount: amount, requisiteType: requisiteType, parentId: id}
        )
        if (prereq2[3].trim() !== '') {
        ...
        const newId = await db.insertPrerequisites(fastify,
                { ...nullPrerequisite, amount: 1, requisiteType: 'antireq', parentId: id }
            )
        if (prereq3[1].trim() !== '') {
        ...
        db.insertLevelPrerequisites(fastify,
            { ...nullPrerequisite, level: prereq4[2] === '' ? level : level + '+', requisiteType: 'prereq', parentId: id}
        )
        return [null, coreqId]
    }
    else if (prereq5) {
        ...
        await db.insertPrerequisites(fastify,
            { ...nullPrerequisite, amount: amt, grade: grade, requisiteType: 'prereq', parentId: id}
        )
        return [id, coreqId]
    }
    else if (prereq6) {
        ...
        let pseudoCourses: PseudoCourse[] = []
        ...
        for (let i = 0; i < subjects.length; ++i) {
            pseudoCourses.push({ ...nullPseudoCourse, subject: subjects[i], 
                minCatalogNumber: minCatalogNumber, maxCatalogNumber: maxCatalogNumber,
            component: component })
        }
        db.insertCoursePrerequisites(fastify,
            { ...nullPrerequisite, units: units, requisiteType: 'prereq', parentId: id, pseudoCourses: pseudoCourses})
        return [null, coreqId]
    }
    else if (prereq7) {
        ...
        if (prereq7[1] === '') {
            ...
            db.insertProgramPrerequisites(fastify,
                { ...nullPrerequisite, average: average, averageType: averageType, parentId: id,
                    programs: null, faculties: null
                }
            )
            return [null, coreqId]
        }
            ...
            db.insertProgramPrerequisites(fastify,
                { ...nullPrerequisite, average: average, averageType: averageType, parentId: id,
                    programs: programs, faculties: null
                 }
            )
            return [null, coreqId]
        }
        ...
        db.insertOtherPrerequisites(fastify, 
            { ...nullPrerequisite, other: requirement, parentId: id })
        return [null, id]
    }
}

async function parseCourses (fastify: FastifyInstance, courses: string[], requisiteType: requisiteType, id: number) {
        ...
        if (/(?<=(?:\/| or )[A-Z]{0,6}\s*[0-9]{1,3}[A-Z]?)(?:| .*)$/g.exec(courses[i])) {
            ...
            for (let j = 0; j < subcourses.length; ++j) {
                ...
                if (/Topic [0-9]*:?[^)]*/g.exec(courses![i].trim())) {
                    ...
                    db.insertPseudoCourses(fastify, id,
                        { ...nullPseudoCourse, subject: subject, catalogNumber: catalog }, 'Prerequisite')
                }
            }
        }
        else {
...
async function parseTopics (fastify: FastifyInstance, topics: string[], requisiteType: requisiteType, subject: string, catalog: string, id: number) {
    for (let i = 0; i < topics.length; ++i) {
        const topic = topics[i].replace(/Topic [0-9]*:?/g, '').trim()
        db.insertPseudoCourses(fastify, id,
            { ...nullPseudoCourse, subject: subject, catalogNumber: catalog, topic: topic}, 'Prerequisite')
    }
}

async function parseTerms (fastify: FastifyInstance, terms: string[], requisiteType: requisiteType, subject: string, catalog: string, id: number) {
    ...
    for (let i = 1; i < terms.length; ++i) {
        ...
        db.insertPseudoCourses(fastify, id,
            { ...nullPseudoCourse, subject: subject, catalogNumber: catalog, term: term}, 'Prerequisite')
    }
}
