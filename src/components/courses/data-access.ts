...
export async function fetchCourse(fastify: FastifyWithTypeProvider, courseid: string): Promise<Course[]> {
    const gotCourse = await fastify.pg.query<Course>('SELECT * FROM courses WHERE courseid = $1', [courseid])
...
export async function fetchPrograms(fastify: FastifyWithTypeProvider): Promise<Program[]> {
    const gotList = await fastify.pg.query<Program>('SELECT * FROM programs')
    return gotList.rows
}

export async function fetchProgram(fastify: FastifyWithTypeProvider, id: number): Promise<Program> {
    const gotProgram = await fastify.pg.query<Program>('SELECT * FROM programs WHERE id = $1', [id])
